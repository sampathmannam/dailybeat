# DailyBeat — Honest Release-Candidate Rating (v3.6.0)

This assessment covers `hardening/end-to-end-reliability`. It reflects source review, 164 JVM
test methods, 16 Android test methods on an API 34 emulator, 10 Python tests, Android Lint, APK
assembly, adversarial GPS/cloud/backup cases, and CI failure-evidence capture. The live cloud
backup test skips when CI credentials are intentionally absent.

It is not a claim that DailyBeat is bug-free or field-proven. The isolated QA package is ready for
the connected-phone gate, but a multi-day physical-device trial is still required before calling
v3.6 production-proven.
...
| Release engineering | 8.8 | Pinned actions, wrapper validation, parallel verify/instrumentation gates, permanent-certificate verification, main/tag/version checks, versioned APK, checksum, timeouts, and uploaded evidence. Staged rollout and automated rollback are not implemented. |
...
1. Run `scripts/mac_phone_e2e.sh` on the connected Motorola and retain its device, package,
   screenshot, launch, PID, and logcat evidence.
...
4. Run the owner-only Supabase backup/restore contract against a non-production test account and
   verify restore onto a second physical device.
...
10. Use a staged rollout before promoting v3.6.0 to every device.
...
| Deterministic verification | APK assembly, 164 JVM tests, Android Lint, 10 Python tests | GitHub `verify` job |
