...
If a device or account is lost or suspected compromised: report it immediately through the normal command/radio/phone chain, notify the subdivision supervisor, disable the membership and Auth user, revoke sessions, remove the device from the authorized installation inventory, preserve relevant audit events, rotate exposed credentials, and document the decision to restore or invalidate pending evidence. If a service-role credential is exposed, rotate it immediately and review every privileged event during the exposure window. Operational emergencies and device/evidence incidents use the normal command/radio/phone chain; privacy, access, correction, export, deletion, or grievance requests go through the subdivision supervisor's existing official Department channel. Neither route is a separate PatrolGrid support desk.
